  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_ENDPOINT}/product-origins?${query}`,
      )
        ...
        <CCard className="mb-4">
          <CCardBody>
            <div className="d-block d-md-flex justify-content-between">
              ...
              <Link to="/product-origin/add">
                <CButton color="info" className="text-white w-100">
                  <FontAwesomeIcon icon={faPlus} /> <strong>Xuất xứ</strong>
                </CButton>
              </Link>
            </div>
          </CCardBody>
                  ...
                  <CTableRow key={item.id}>
                    ...
                    <CTableDataCell>
                      <CDropdown>
                        ...
                        <CDropdownMenu>
                          <CDropdownItem href={`/product-origin/edit?id=${item.id}`}>
