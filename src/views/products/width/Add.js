...
const Add = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (id === null) {
      // Add
      axios
        .post(`${process.env.REACT_APP_API_ENDPOINT}/product-widths`, {
          data: data,
        })
      ...
      axios
        .put(`${process.env.REACT_APP_API_ENDPOINT}/product-widths/${id}`, {
          data: data,
        })
        .then((response) => toast.success('Thao tác thành công'))
        .catch((error) => {
  ...
  const fetchWidthData = async () => {
    ...
    const response = await axios.get(`
      ${process.env.REACT_APP_API_ENDPOINT}/product-widths/${id}?${query}`)
